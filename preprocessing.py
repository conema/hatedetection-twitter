...
def normalize_slangs(tweet):
  dictionary = {'u':'you', 'ppl':'people', 'bc':'because', 'bs': 'bullshit', 
                ...
                'nutshel': 'nutshell', '#bihday': '#birthday', 'exactli': 'exactly', 'fishi': 'fishy', 'easili': 'easily', 'Ima': 'i am going to',
                'Y’all': 'you all'}
...
def preprocess_tweets(tweets):
    ...
    df['preprocessed_tweet'] = df['tweet'].apply(lambda tw : ' '.join([tweet for tweet in tw.split() if not tweet.startswith("@")]))        #remove @username
